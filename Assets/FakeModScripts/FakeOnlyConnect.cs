...
public class FakeOnlyConnect : ImpostorMod
{
    ...
    [SerializeField]
    private Texture[] hieroglyphTextures;
    [SerializeField]
    private TextMesh teamText;
    [SerializeField]
    private Texture susTexture;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < buttonObjs.Length; i++)
            buttonObjs[i].GetComponent<MeshRenderer>().material.mainTexture = hieroglyphTextures[rndSynbols[i]];
        teamText.text = teamNames[Rnd.Range(0, teamNames.Length)];
        Case = Rnd.Range(0, 2);
        switch (Case)
        {
            case 0:
                dupeSymbols:
                var rnd1 = Rnd.Range(0, 6);
                var rnd2 = Rnd.Range(0, 6);
                if (rnd1 == rnd2)
                    goto dupeSymbols;
                var shuff = Enumerable.Range(0, 6).ToArray().Shuffle();
                shuff[rnd1] = shuff[rnd2];
                flickerObjs.Add(buttonObjs[rnd1]);
                flickerObjs.Add(buttonObjs[rnd2]);
                for (int i = 0; i < buttonObjs.Length; i++)
                    buttonObjs[i].GetComponent<MeshRenderer>().material.mainTexture = hieroglyphTextures[shuff[i]];
                break;
            case 1:
                var sus = Rnd.Range(0, 6);
                buttonObjs[sus].GetComponent<MeshRenderer>().material.mainTexture = susTexture;
                flickerObjs.Add(buttonObjs[sus]);
                break;
        }
